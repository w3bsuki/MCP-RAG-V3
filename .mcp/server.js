...
async function generateEmbedding(text) {
    ...
    const embedding = new Array(EMBEDDING_DIM).fill(0).map((_, i) => Math.sin((hash + i) * 0.1) * 0.1);
    return embedding;
}
...
server.tool("mcp__ragStore__search", "Search project memory for patterns/solutions", {
    ...
    try {
        ...
        const filtered = data.filter((item) => item.content.toLowerCase().includes(query.toLowerCase()) &&
